...
const Product = ({ price, rating, title, thumbnail, stock , discountPercentage }) => {
  const [{ basket }, dispatch] = UsestateValue();

  const Add = () => {
    if(stock < 50){
      prompt("hurry! only a few items left");
    }
    dispatch({
      type: "ADD",
      payload: {
  ...
  return (
    <>
      <div className="bg-[#141414] lg:w-[90%] w-full  text-white rounded-md">
        ...
        <div className="text-[13px] p-1  text-white">{title}</div>
        <div className="flex  justify-between p-1  ">
          <div className="flex text-[14px]  items-baseline  space-y-3 p-1 flex-col">
            <div className="flex items-center gap-1">
              ...
              <span className="md:text-[13px] text-[8px]  "> {price}</span>
            </div>
            <span className="md:text-[13px] text-[8px]  ">
              {" "}
              rating :{rating}
            </span>
            <span> discount:   {discountPercentage}</span>

            <span className="md:text-[12px]  text-[8px] text-green-500">
              stock :{stock}
            </span>
          </div>
          <div className="relative mb-6">
            ...
            {basket?.filter((data) => data?.title === title)?.length ? (
              <span className="bg-black top-[24px] right-[2px] w-6 h-6 rounded-full  flex justify-center text-sm absolute   ">
                {basket?.filter((data) => data?.title === title)?.length}
              </span>
            ) : (
              ""
